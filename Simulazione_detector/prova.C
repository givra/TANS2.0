...
void prova(){
  ...
  int b1=xa->FindBin(2.6); //per eta -2.//per m 2.5
  int b2=xa->FindBin(70.4821); //per eta 2.//per m 70.5
  int nobins=b2-b1+1;
 cout<<"nobins "<<nobins<<endl;
  float Xlow=xa->GetBinLowEdge(b1); //eta -2.04//m
  float Xhig=xa->GetBinUpEdge(b2); //eta 2.04//m
  ...
  TH1D* hist = new TH1D ("h","distribuzione molt" , nobins, Xlow, Xhig);
  hist->SetMinimum(0);
  ...
  hist->Fill(((Xhig-Xlow)*(Xt-1)/(nobins))+Xlow);   //-((Xhig-Xlow)/(2*nobins)));-((Xhig-Xlow)/2*nobins));
  ...
  cout<<"numero bin "<<nobins<<"xlow "<<xlow<<"xhig "<<xhig<<endl;
  //TH1F* heta2 = new TH1F("heta2","#eta distribution 2",nobins,xlow,xhig);
  TH1F* hm = new TH1F("hm","molteplicity distribution",nobins,xlow,xhig);
  int j=1;
  //for(int i=b1;i<=b2;i++)heta2->SetBinContent(j++,disteta->GetBinContent(i));
  for(int i=b1;i<=b2;i++)hm->SetBinContent(j++,disteta->GetBinContent(i));
  //heta2->Draw();
  //new TCanvas();
  //disteta->Draw();
